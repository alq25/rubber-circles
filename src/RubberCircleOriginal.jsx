...
export default function RubberCircleOriginal({ shapeType = "blob", size = 300 }) {
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, margin: 15 }}>
      {layers.map((layer, idx) => {
        ...
        const frequency = 0.8 + layer * 0.3;
        const amplitude = 0.4 - layer * 0.06;

        // Pick a random pinch side per layer with longer timing but much faster movements
        const pinchSide = Math.floor(layerTime * 0.08 + phases[idx] * 3) % 4;
        const pinchProgress = (layerTime * 0.5 + phases[idx]) % 1; // Much faster pinch movements
        ...
        const baseMemoryEffect = Math.exp(-pinchProgress * 2.5); // Smooth snap-back
        const enhancedMemoryEffect = layer >= 3 ? Math.exp(-pinchProgress * 2) : baseMemoryEffect; // Removed oscillations
        const memoryEffect = enhancedMemoryEffect;
        const hysteresisEffect = layer >= 3 ? Math.sin(pinchProgress * Math.PI * 2 + phases[idx] * 0.6) * 0.15 : Math.sin(pinchProgress * Math.PI * 2 + phases[idx] * 0.4) * 0.1; // Reduced amplitude
        ...
        // Varied rotation for organic movement with faster cycles
        const rotate = (1.5 + layer * 1.2) * Math.sin(layerTime * 0.4 + phases[idx] * 0.5) * 0.8; // Faster, varied rotation
